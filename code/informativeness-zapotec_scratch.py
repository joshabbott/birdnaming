...
hummingbird_pcs = {}
for bird in hummingbirds:
	bird_features = np.array(pc_data[['PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8','PC9']][pc_data['Binomial'] == bird.replace(' ','_')])
	if len(bird_features) > 0:
		hummingbird_pcs[bird] = bird_features


bird_list = list(hummingbird_pcs.keys())

simMatrix = createSimMatrix(bird_list,hummingbird_pcs)
...
# create need probs (uniform vs. ebird relative freq)
uniform_need_probs = {bird: (1.0/len(bird_list)) for bird in bird_list}

# evalue = ERE(lmap,simMatrix,uniform_need_probs)
# print("Uniform need probs: ", evalue)
# In [52]: ERE(lmap,simMatrix,uniform_need_probs)                                                                                                                       
# Out[52]: 3.6418178563638186




# randomize set of folk_specific labels and compute E
# first remake lmap

hummingbirds_labels = []
for bird in bird_list:
	hummingbirds_labels.append(list(df[df['species'] == bird]['folk_specific'])[0])

hummingbirds_labels_orig = hummingbirds_labels

lmap = {}
i=0
for bird in bird_list:
	lmap[bird] = hummingbirds_labels_orig[i]
	i+=1

evalue = ERE(lmap,simMatrix,uniform_need_probs)
print("Uniform need probs (Zapotec hummingbirds): ", evalue)
# In [66]: evalue = ERE(lmap,simMatrix,uniform_need_probs) 
#     ...: print("Uniform need probs: ", evalue)                                                                                                                
# Uniform need probs:  3.64181785636
...
print("Freq need probs (Zapotec hummingbirds): ", evalue_freq)


# shuffle labels and recompute E
evalue_rands = computeRandomShuffles(bird_list,hummingbirds_labels,simMatrix,uniform_need_probs)
