...
df_mf <- df  %>% 
  ...
  mutate(plotgen =factor(folk_generic)) %>% 
  mutate(plotgen =fct_relabel(plotgen, slab) )
...
morphospace_plot <- df_mf %>% 
  ...
  ggplot(aes(x = e1, y = e2, shape=plotgen, color = plotgen)) +
...
lmlumping <- lm(
  formula = group_size_folk_generics ~ logfreq + logmass + density, 
  data = df_named)
...
logitmodcompound <- glm(
  formula = is_compound ~ logfreq + logmass + density, 
  family = binomial,
  data = df_named)
...
lmnamelength<- lm(
  formula = logname_length ~ logfreq + logmass + density, 
  data = df_named)
